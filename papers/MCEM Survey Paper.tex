...
%
\begin{proposition}
    Let $\Delta \hat{Q}(\theta_k|\theta_{k-1}) = \hat{Q}(\theta_k|\theta_{k-1}) - \hat{Q}(\theta_k|\theta_{k-1})$. Define $\Delta Q(\theta_k|\theta_{k-1})$ similarly. Let $M_k$ be the Monte Carlo size at iteration $k$. Then
    %
    \begin{align}
        \sqrt{M_k} \left[ \Delta \hat{Q}(\theta_k|\theta_{k-1}) - \Delta Q(\theta_k|\theta_{k-1}) \right] \rightsquigarrow N(0, \Sigma_k)
    \end{align}
\end{proposition}
%
