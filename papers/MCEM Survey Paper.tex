...
We illustrate the SAEM methods of \citet{Gu98I} and \citet{Jan06} on our blood type example. For the method of \citeauthor{Gu98I}, we use $\alpha_k = k^{-1}$ (a popular choice), a Monte Carlo sample size of 10 at each iteration, and 50 iterations. Our final parameter estimate is $\hat{p} = 0.301$ and $\hat{q} = 0.128$. This estimate is close to the MLE, but differs more than the MCEM methods. It is worth noting however that, while the number of Monte Carlo samples used to compute the final estimate under SAEM is 500, which is comparable to the final Monte Carlo sizes of MCEM methods, 500 is also the total number of Monte Carlo draws for SAEM. MCEM methods on the other hand, require hundreds of Monte Carlo samples at every iteration, and thus have a much higher computational cost. \hl{Compare total number of MC samples across methods.}


\begin{figure}
    \centering
    \caption{Trajectory of estimates for $p$ and $q$, from the SAEM method of \citeauthor{Gu98I}. Horizontal dashed lines give the maximum likelihood estimates.}
    \label{fig:blood_SAEM_traj}
    \includegraphics[width=0.75\textwidth]{../plots/Blood_Type/SAEM_Traj.pdf}   
\end{figure}
