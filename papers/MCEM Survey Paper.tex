...
\begin{align}
    I(\hat{\theta}) &= \begin{bmatrix}
        3.94 \cdot 10^7 & 1.20 \cdot 10^7\\
        1.20 \cdot 10^7 & 5.95 \cdot 10^7
    \end{bmatrix}\\
    \hat{\Sigma}_\mathrm{MLE} &= \begin{bmatrix}
        2.70 \cdot 10^{-8} & -5.46 \cdot 10^{-9}\\
        -5.46 \cdot 10^{-9} & 1.79 \cdot 10^{-8}
    \end{bmatrix}
\end{align}
...
\end{align}
%
Starting with $\theta_0 = (1/3, 1/3)$ corresponding to equal proportions of the three alleles, Figure \ref{fig:blood_EM_traj} gives trajectories of the EM estimates for $p$ and $q$ using the data in Figure \ref{tab:blood_type}. These estimates converge quite quickly the maximizer of the observed data likelihood.
%
\begin{figure}
    \centering
...
In order to apply the MCEM algorithm to estimate allele frequencies in the blood type problem, we must specify the number of iterations and the Monte Carlo size for each iteration. Starting conservatively, we use 50 iterations and an MC size of 100. Figure \ref{fig:blood_naive_MCEM_traj} gives trajectories of the MCEM estimates for $p$ and $q$. These estimates appear to stabilize quickly, and with a high degree of precision. Taking the output from the final iteration as our estimate, we get $\hat{p} = 0.315$ and $\hat{q} = 0.170$.

\begin{figure}
    \centering
    \caption{Trajectory of MCEM estimates for $p$ and $q$ for the blood type example.}
    \label{fig:blood_naive_MCEM_traj}
    \includegraphics[width=0.5\textwidth]{../plots/Blood_Type/naive_MCEM_traj.pdf}
\end{figure}
