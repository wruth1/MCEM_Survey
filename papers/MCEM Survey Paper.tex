...
Consider the problem of gene frequency estimation. That is, we seek to estimate the frequencies of different alleles for a particular gene based on a sample of phenotypes. We closely follow Example 2.4 from \citet{McL08}\footnote{Although these authors do give data in their example, there is little context around this data, and I have been unable to locate more information from their references. I have opted instead to use my own dataset.}

Consider the gene for ABO blood group (see Chapter 5 of \citealp{Dea05}, for a more thorough overview). There are three alleles for this gene: A, B and O. Allele O is recessive, which alleles A and B exhibit co-dominance. That is, genotypes AO and AA encode blood type A, genotypes BO and BB encode blood type B, genotype OO encodes blood type O, and genotype AB encodes blood type AB. Suppose that we seek to estimate the proportion of each allele within a population, based on a sample of individuals' phenotypes. \citet{Fuj78} report blood types of 4,464,349 people in Japan collected between 1964 and 1975, see Figure \ref{tab:blood_type}. 
...
Note that we can write $Y$ in terms of $X$. Specifically, $Y_1 = X_1$, $Y_2 = X_2 + X_3$, $Y_3 = X_4 + X_5$ and $Y_4 = X_6$. This corresponds to summing components of $X$ which correspond to the same blood type.
...
\begin{appendices}
    ...
    The observed data likelihood for our model can be written as follows:
    %
    \begin{align}
    ...
    \begin{align}
        ...
        \partial_p \ell(\theta; Y) &= - \frac{2 y_1}{r}  + \frac{2r y_2}{p^2 + 2pr}  - \frac{2q y_3}{q^2 + 2qr}  + \frac{y_4}{p}\mathrm{, and} \label{eq:gene_obs_score1}\\
    ...
    Differentiating $\ell$ again and multiplying by $-1$ gives the observed data information matrix.
    %
    \hl{Add this later}
