...
\hl{I'm not super comfortable with this material. I want to acknowledge it and use it to motivate the more complicated adjustments to make MCEM work with MCMC, but I also don't want to say anything silly.} Limit theory for estimators based on MCMC sampling are more complicated than the similar theory for estimators based on iid or importance sampling (\hl{SMC?}). This additional complexity stems from the dependence between draws from an MCMC sampler. While convergence for iid and importance sampling can be established using the Law of Large Numbers and the classical Central Limit Theorem, similar results for MCMC sampling make use of the Ergodic Theorem and Markov Chain Central Limit Theorem. 

Efforts have been made in the MCEM literature to address this additional complexity and account for dependence in our Monte Carlo sample.
